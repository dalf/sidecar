...
use self::charabia::{Segment, Tokenize};
...
mod tests {
    ...
    fn it_works_fr_segment() {
        ...
        while let Some(token) = token_stream.next() {
            ...
            println!(
                "{} ({},{}) {} {}",
                token.position, token.offset_from, token.offset_to, token.text, token.position_length
            );
        }
    }
    ...
    fn it_works_zh() {
        ...
        let mut token_stream = tokenizer
            .token_stream("张华考上了北京大学；李萍进了中等技术学校；我在百货公司当售货员：我们都有光明的前途");
        let mut tokens = Vec::new();
        let mut token_text = Vec::new();
        while let Some(token) = token_stream.next() {
            tokens.push(token.clone());
